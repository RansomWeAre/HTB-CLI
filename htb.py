...
def cmd_profile():
    ...
    print(
        f"  {C.BOLD}Rank     :{C.RESET} {C.YELLOW}{u.get('rank', '?')}{C.RESET}  •  Points: {C.CYAN}{u.get('points', '?')}{C.RESET}"
    )
    print(f"  {C.BOLD}Respects :{C.RESET} {u.get('respects', '?')}")
    print(
        f"  {C.BOLD}Owns     :{C.RESET} {C.GREEN}{u.get('user_owns', 0)} user{C.RESET}  /  {C.RED}{u.get('system_owns', 0)} root{C.RESET}"
    )
    team = u.get("team")
    if team:
